package jaysan.wildfly.gestion.wildflygestion.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "wildflygestion-ear.ear");

        // Current directory is the root of the "wildflygestion-web" project. Go up one level
        // and enter the "wildflygestion-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../wildflygestion-ejb/target/wildflygestion-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "wildflygestion-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/wildflygestion-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "wildflygestion-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("jaysan.wildfly.gestion.wildflygestion.test");
